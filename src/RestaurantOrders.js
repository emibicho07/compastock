                  ...
                  <div className="order-items">
                    {orderItems.map(item => (
                      <div key={item.productId} className="order-item">
                        ...
                        <button onClick={() => updateQuantity(item.productId, 0)} className="remove-button">🗑️</button>
                      </div>
                    ))}
                  </div>
              ...
              {orders.map(order => (
                <div key={order.id} className={`order-card ${order.isUrgent ? 'urgent' : ''}`}>
                  <div className="order-header">
                    <h4>{order.isUrgent ? '🚨 Pedido Urgente' : '📋 Pedido Semanal'}</h4>
                  ...
                  <div className="order-items-summary">
                    ...
                    <ul>
                      {order.items.slice(0, 3).map(item => (
                        <li key={item.productId}>{item.productName} ({item.quantity} {item.unit})</li>
                      ))}
